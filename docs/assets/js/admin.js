...
async function removeCsvRow(path, id, label) {
  for (let attempt = 0; attempt < 3; attempt++) {
    const file = await githubApi(`/repos/${GITHUB_REPOSITORY}/contents/${path}?ref=${encodeURIComponent(GITHUB_BRANCH)}`);
    const text = base64ToText(file.content);
    const rows = parseCSV(text).filter((row) => row.id !== id);
    const headers = text.split(/\r?\n/, 1)[0];
    const updated = `${headers}\n${rows.map((row) => headers.split(",").map((header) => csvField(row[header])).join(",")).join("\n")}\n`;
    try {
      await githubApi(`/repos/${GITHUB_REPOSITORY}/contents/${path}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: `Remove ${label}`, content: textToBase64(updated), sha: file.sha, branch: GITHUB_BRANCH }) });
      return;
    } catch (error) {
      if (error.status !== 409 || attempt === 2) throw error;
    }
...
function renderAll() {
  ...
  const manageEl = document.getElementById("manageItems");
  manageEl.innerHTML = [
  ...
  manageEl.querySelectorAll("button").forEach((button) => button.addEventListener("click", async () => {
    ...
    button.disabled = true;
    try {
      await removeCsvRow(button.dataset.removePath, button.dataset.removeId, button.dataset.removeLabel);
